...
dittoViz_scatterPlotApp <- function(data_list = NULL) {
    ...
    ui <- fluidPage(
        ...
        sidebarLayout(
            ...
            mainPanel(
                ...
                # --- Plot ---
                dittoViz_scatterPlotOutputUI("active_plot"),
                hr(),
    ...
    server <- function(input, output, session) {
        ...
        output$plot_inputs_ui <- renderUI({
            ...
            dittoViz_scatterPlotInputsUI("active_plot", rv$datasets[[input$plot_select]],
                title = h3(paste(input$plot_select, "Settings"))
            )
        })

        # ---- Plot Module ----
        dittoViz_scatterPlotServer("active_plot", data = filtered_data)
    }
    shinyApp(ui, server)
}
