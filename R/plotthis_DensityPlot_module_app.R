...
plotthis_DensityPlotApp <- function(data_list = NULL) {
    ...
    ui <- fluidPage(
        ...
        sidebarLayout(
            ...
            mainPanel(
                ...
                # --- Plot ---
                plotthis_DensityPlotOutputUI("active_plot"),
                hr(),
    ...
    server <- function(input, output, session) {
        ...
        output$plot_inputs_ui <- renderUI({
            ...
            plotthis_DensityPlotInputsUI("active_plot", rv$datasets[[input$plot_select]],
                title = h3(paste(input$plot_select, "Settings"))
            )
        })

        # ---- Plot Module ----
        plotthis_DensityPlotServer("active_plot", data = filtered_data)
    }
    shinyApp(ui, server)
}
