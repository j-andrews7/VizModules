...
#' that matches the `id` used in the `ViolinPlotServer()` and `ViolinPlotOutputUI()` functions.
#'
...
#' [vizModules::ViolinPlotOutputUI()], [vizModules::ViolinPlotServer()], [vizModules::ViolinPlotApp()]
#' @examples
#' library(vizModules)
#' data(mtcars)
#' ViolinPlotInputsUI("ViolinPlot", mtcars)
ViolinPlotInputsUI <- function(id, data, defaults = NULL, title = NULL, columns = 2) {
    ns <- NS(id)
    ...
    inputs <- list(
        ...
        "Adjustments" = tagList(
            ...
            switchInput(ns("add.box"), "Add Box: ", value = FALSE, onLabel = "Box", offLabel = "No Box"),
...
#' @export
#' @author Jacob Martin
ViolinPlotOutputUI <- function(id) {
    ns <- NS(id)
    jqui_resizable(
