...
volcanoPlotInputsUI <- function(id, data, defaults = NULL, title = "Volcano Settings", columns = 2) {
    ...
    if (!"x.by" %in% names(defaults)) defaults$x.by <- "log2FoldChange"
    if (!"y.by" %in% names(defaults)) defaults$y.by <- "padj"
    ...
    if (!"hover.data" %in% names(defaults)) defaults$hover.data <- c("symbol", "log2FoldChange", "padj")
