...
#'   \item \code{base} - Base plot type box/violin/bar (fixed to "box" in this module)
...
#'   \item \code{add_errorbar} - Add error bars (only for base = "bar"; not implemented)
#'   \item \code{errorbar_color} - Error bar color (not implemented)
#'   \item \code{errorbar_width} - Error bar cap width (not implemented)
#'   \item \code{errorbar_linewidth} - Error bar line width (not implemented)
