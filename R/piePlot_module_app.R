...
#' aggregated `example_sales` and `example_population` datasets. Uploaded Excel files are added
...
piePlotApp <- function(data_list = NULL) {
    ...
    server <- function(input, output, session) {
        ...
        output$plot_inputs_ui <- renderUI({
            ...
            piePlotInputsUI("active_plot", rv$datasets[[input$plot_select]],
                title = h3(paste(input$plot_select, "Settings"))
            )
        })

        # ---- Plot Module ----
        piePlotServer("active_plot", data = filtered_data)
    }
    shinyApp(ui, server)
}
