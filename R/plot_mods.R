#' Create axis styling lists from input parameters
#'
#' Helper function to construct xaxis_style and yaxis_style lists from
#' axis styling parameters. Reduces code duplication across plotting modules.
#'
#' @param axis.font.size Numeric. Font size for axis titles.
#' @param font.type Character. Font family for axis titles.
#' @param text.colour Character. Color for axis title text.
#' @param axis.showline Logical. Whether to show axis lines.
#' @param axis.mirror Logical or character. Whether to mirror axis lines.
#' @param axis.linecolor Character. Color of axis lines.
#' @param axis.linewidth Numeric. Width of axis lines.
#' @param axis.tickfont.size Numeric. Font size for axis tick labels.
#' @param axis.tickfont.color Character. Color of axis tick labels.
#' @param axis.tickfont.family Character. Font family for axis tick labels.
#' @param axis.tickangle.x Numeric. Angle of x-axis tick labels.
#' @param axis.tickangle.y Numeric. Angle of y-axis tick labels.
#' @param axis.ticks Character. Position of axis ticks (e.g., "outside", "inside").
#' @param axis.tickcolor Character. Color of axis ticks.
#' @param axis.ticklen Numeric. Length of axis ticks.
#' @param axis.tickwidth Numeric. Width of axis ticks.
#' @param x.title Character or NULL. Optional x-axis title (for linePlot).
#' @param y.title Character or NULL. Optional y-axis title (for linePlot).
#' @param autorange.x Logical or character. Optional autorange setting for x-axis (for linePlot).
#' @param autorange.y Logical or character. Optional autorange setting for y-axis (for linePlot).
#'
#' @return A list with two elements: `xaxis_style` and `yaxis_style`, each
#'   containing a named list of axis styling parameters suitable for plotly layout.
#'
#' @author Jared Andrews
#' @rdname INTERNAL_create_axis_styles
#' @keywords internal
.create_axis_styles <- function(
    axis.font.size = 18,
    font.type = "Arial",
    text.colour = "#000000",
    axis.showline = TRUE,
    ...
    # Base styling common to both axes
    base_style <- list(
        ...
        tickfont = list(
            size = axis.tickfont.size,
            color = axis.tickfont.color,
            family = axis.tickfont.family
        ),
        ticks = axis.ticks,
    ...
    xaxis_style <- base_style
    xaxis_style$tickangle <- axis.tickangle.x

    # Add title if provided (for linePlot compatibility)
    if (!is.null(x.title)) {
        xaxis_style$title <- x.title
    } else {
        # Standard module case: add title with font styling
        xaxis_style$title <- list(
            font = list(
                size = axis.font.size,
                family = font.type,
                color = text.colour
    ...
    # Add autorange if provided (for linePlot compatibility)
    if (!is.null(autorange.x)) {
        xaxis_style$autorange <- autorange.x
    }

    # Y-axis specific styling
    yaxis_style <- base_style
    ...
    # Add autorange if provided (for linePlot compatibility)
    if (!is.null(autorange.y)) {
        yaxis_style$autorange <- autorange.y
    }

    list(xaxis_style = xaxis_style, yaxis_style = yaxis_style)
}
