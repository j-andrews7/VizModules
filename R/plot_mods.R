...
.apply_subplot_axis_styling <- function(fig, xaxis_style, yaxis_style) {
    ...
    # Equalize facet panel sizes. ggplotly allocates extra space to edge panels
    # (first/last column, first/last row) to absorb axis labels, strip titles, and
    # mirror borders, leaving interior panels noticeably narrower/shorter. Post-
    # process the axis domains so every panel within a row has the same width and
    # every row has the same height (and similarly for shared-x layouts).
    fig <- .equalize_facet_domains(fig)

    fig
}

#' Equalize plotly subplot axis domains so faceted panels have uniform sizes
#'
#' ggplotly's facet layout reserves extra width for the first and last columns
#' (axis labels, mirror border) and extra height for the first and last rows
#' (strip titles, x-axis tick labels). The result is subplots whose domain
#' widths/heights differ by 5-50 percent within a single figure. This helper
#' redistributes the axis domains so that all panels sharing a row (i.e. anchored
#' to the same y-axis) have equal width, and all rows sharing a column anchor
#' have equal height. Existing inter-panel gaps are preserved (averaged).
#'
#' Safe to call on non-faceted figures; a no-op when fewer than two axes share
#' an anchor group.
#'
#' @param fig A plotly figure object, typically produced by \code{ggplotly()}.
#'
#' @return The plotly figure with equalized axis domains.
#'
#' @author Jared Andrews
#' @rdname INTERNAL_equalize_facet_domains
#' @keywords internal
.equalize_facet_domains <- function(fig) {
    if (is.null(fig) || is.null(fig$x) || is.null(fig$x$layout)) {
        return(fig)
    }
    ...
    compute_group_updates <- function(axis_names) {
        ...
        # form one row of panels; all y-axes anchored to the same x-axis form
        # one column of rows that should share a height).
        anchors <- vapply(axes, function(a) {
            anc <- a$anchor
            if (is.null(anc) || length(anc) == 0) NA_character_ else as.character(anc)
        }, character(1))

        for (grp in unique(anchors)) {
            if (is.na(grp)) next
            grp_names <- axis_names[anchors == grp]
            if (length(grp_names) < 2) next

            grp_axes <- fig$x$layout[grp_names]
            # Keep only axes with valid 2-element domains.
            valid <- vapply(grp_axes, function(a) {
                is.numeric(a$domain) && length(a$domain) == 2 && all(is.finite(a$domain))
            }, logical(1))
            if (sum(valid) < 2) next
            grp_names <- grp_names[valid]
            grp_axes <- grp_axes[valid]

            doms <- do.call(rbind, lapply(grp_axes, function(a) a$domain))
            ord <- order(doms[, 1])
            doms <- doms[ord, , drop = FALSE]
            grp_names <- grp_names[ord]

            k <- nrow(doms)
            span_lo <- doms[1, 1]
            span_hi <- doms[k, 2]
            gaps <- doms[-1, 1] - doms[-k, 2]
            gap <- if (length(gaps) > 0) max(0, mean(gaps)) else 0
            new_w <- ((span_hi - span_lo) - (k - 1) * gap) / k
            if (!is.finite(new_w) || new_w <= 0) next
