...
#' Adjust vertical spacing between subplot rows in a plotly figure
#'
#' When using ggplotly-converted faceted plots, the vertical gaps between
#' subplot rows can be too tight. This function detects subplot rows by
#' their y-axis domain values and redistributes vertical space to add
#' larger gaps between rows. It also updates annotation positions
#' (e.g., facet strip labels) to match the new layout.
#'
#' @param fig A plotly figure object.
#' @param spacing Numeric. The gap to add between subplot rows (as a
#'   fraction of total plot height, e.g. 0.08 = 8\%). Default is 0.08.
#'
#' @return The modified plotly figure with adjusted vertical subplot spacing.
#'
#' @author Jared Andrews
#' @rdname INTERNAL_adjust_subplot_vertical_spacing
#' @keywords internal
.adjust_subplot_vertical_spacing <- function(fig, spacing = 0.08) {
    if (is.null(fig) || is.null(fig$x) || is.null(fig$x$layout)) {
        return(fig)
    }
    ...
    # Get original row info (height of each row)
    row_info <- lapply(unique_rows, function(row_bottom) {
        idx <- which(abs(rounded_bottoms - row_bottom) < 0.001)[1]
        name <- yaxis_names[idx]
        domain <- fig$x$layout[[name]]$domain
        list(bottom = domain[1], top = domain[2], height = domain[2] - domain[1])
    })

    # Calculate new domains with specified spacing between rows
    total_original_height <- sum(vapply(row_info, function(r) r$height, numeric(1)))
    total_new_gaps <- spacing * (n_rows - 1)
    available_height <- 1.0 - total_new_gaps
    scale_factor <- available_height / total_original_height

    # Build new domains bottom-up
    new_row_domains <- list()
    current_bottom <- 0
    for (i in seq_len(n_rows)) {
        new_height <- row_info[[i]]$height * scale_factor
        new_top <- current_bottom + new_height
        new_row_domains[[i]] <- c(current_bottom, new_top)
        current_bottom <- new_top + spacing
    }

    # Build mapping from old row bottom -> new domain for annotation adjustment
    old_to_new <- list()
    for (i in seq_len(n_rows)) {
        old_to_new[[i]] <- list(
            old_bottom = row_info[[i]]$bottom,
            old_top = row_info[[i]]$top,
            new_bottom = new_row_domains[[i]][1],
            new_top = new_row_domains[[i]][2]
        )
    }

    # Apply new domains to all y-axes
    for (j in seq_along(yaxis_names)) {
        name <- yaxis_names[j]
        row_idx <- which(abs(unique_rows - rounded_bottoms[j]) < 0.001)
        if (length(row_idx) > 0) {
            fig$x$layout[[name]]$domain <- new_row_domains[[row_idx[1]]]
        }
    }

    # Adjust annotation positions (facet strip labels) to match new domains
    if (!is.null(fig$x$layout$annotations)) {
        for (k in seq_along(fig$x$layout$annotations)) {
            ...
            if (!is.null(anno$y)) {
                ...
                for (mapping in old_to_new) {
                    if (anno$y >= mapping$old_bottom - 0.01 && anno$y <= mapping$old_top + 0.05) {
                        ...
                        old_range <- mapping$old_top - mapping$old_bottom
                        new_range <- mapping$new_top - mapping$new_bottom
                        if (old_range > 0) {
                            relative_pos <- (anno$y - mapping$old_bottom) / old_range
                            fig$x$layout$annotations[[k]]$y <- mapping$new_bottom + relative_pos * new_range
                        }
                        break
                    }
                }
