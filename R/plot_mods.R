...
#' Apply standard axis post-processing to a plotly figure
#'
#' Combines the repeated post-\code{ggplotly} axis setup into a single call:
#' builds x and y axis style lists via \code{\link{.create_axis_styles}},
#' applies them to all subplot axes via \code{\link{.apply_subplot_axis_styling}},
#' and when \code{facet.by} is non-empty applies axis-title font styling to
#' shared facet annotation titles via \code{\link{.apply_axis_title_to_annotations}}.
#'
#' @param fig A plotly figure object.
#' @param input Shiny input object.
#' @param isolate_fn Function used to isolate Shiny inputs (typically
#'   \code{shiny::isolate}).
#' @param facet.by Character. Name of the column used for faceting. When
#'   non-\code{NULL} and non-empty, shared facet annotation titles are styled.
#' @param ggplot.axis.styling Logical. Passed through to
#'   \code{\link{.create_axis_styles}}. Defaults to \code{TRUE}.
#'
#' @return The modified plotly figure.
#'
#' @author Jacob Martin
#' @keywords internal
#' @rdname INTERNAL_apply_axis_post_processing
.apply_axis_post_processing <- function(fig, input, isolate_fn,
                                        facet.by = NULL,
                                        ggplot.axis.styling = TRUE) {
    xaxis_style <- .create_axis_styles(input, axis_side = "x",
                                       isolate_fn = isolate_fn,
                                       ggplot.axis.styling = ggplot.axis.styling)
...
#' Add reference lines to a plotly figure using Shiny inputs
#'
#' Thin wrapper around \code{\link{.add_reference_lines}} that reads the full
#' set of \code{hline.*}, \code{vline.*}, and \code{abline.*} fields directly
#' from the Shiny input object. This eliminates the 18-argument call block
#' that was previously repeated in every plot module server.
#'
#' @param fig A plotly figure object.
#' @param input Shiny input object containing the \code{hline.*}, \code{vline.*}
#'   and \code{abline.*} fields.
#' @param isolate_fn Function used to isolate Shiny inputs.
#'
#' @return The modified plotly figure with reference lines added.
#'
#' @author Jacob Martin
#' @keywords internal
#' @rdname INTERNAL_add_reference_lines_from_input
.add_reference_lines_from_input <- function(fig, input, isolate_fn) {
    .add_reference_lines(
        fig,
...
#' Apply plotly configuration options from Shiny inputs
#'
#' Wraps \code{\link{.add_plot_config}} and the subsequent
#' \code{do.call(plotly::config, ...)} into a single call, reading
#' \code{download.format} from the Shiny input object.
#'
#' @param fig A plotly figure object.
#' @param input Shiny input object (used for \code{input$download.format}).
#' @param isolate_fn Function used to isolate Shiny inputs.
#' @param include.modebar.buttons Logical. Whether to include shape-drawing
#'   modebar buttons. Defaults to \code{TRUE}.
#' @param facet.by Character or \code{NULL}. Passed through to
#'   \code{\link{.add_plot_config}} to choose the correct edits set.
#'
#' @return The configured plotly figure.
#'
#' @author Jacob Martin
#' @keywords internal
#' @rdname INTERNAL_apply_plot_config
.apply_plot_config <- function(fig, input, isolate_fn,
                               include.modebar.buttons = TRUE,
                               facet.by = NULL) {
    config_list <- .add_plot_config(
        download.format = isolate_fn(input$download.format),
        include.modebar.buttons = include.modebar.buttons,
        facet.by = facet.by
    )
    do.call(plotly::config, c(list(p = fig), config_list))
}


#' Finalize a plotly figure with the standard post-processing pipeline
#'
#' Applies the common post-\code{ggplotly} pipeline used across plot module
#' servers, in order:
#' \enumerate{
#'   \item Axis post-processing (\code{\link{.apply_axis_post_processing}}) —
#'     optional via \code{apply_axis_styling}.
#'   \item Reference lines (\code{\link{.add_reference_lines_from_input}}) —
#'     optional via \code{apply_reference_lines}.
#'   \item Plot configuration (\code{\link{.apply_plot_config}}).
#'   \item Newshape defaults (\code{\link{.apply_plotly_newshape}}).
#' }
#' Use \code{\link{.apply_title_layout}} before calling this helper to apply
#' title styling.
#'
#' @param fig A plotly figure object.
#' @param input Shiny input object.
#' @param isolate_fn Function used to isolate Shiny inputs.
#' @param facet.by Character. Name of the faceting column (or \code{NULL}).
#' @param include.modebar.buttons Logical. Passed through to
#'   \code{\link{.apply_plot_config}}. Defaults to \code{TRUE}.
#' @param ggplot.axis.styling Logical. Passed through to
#'   \code{\link{.apply_axis_post_processing}}. Defaults to \code{TRUE}.
#' @param apply_axis_styling Logical. Whether to apply axis post-processing.
#'   Set to \code{FALSE} for plots constructed natively in plotly (no
#'   \code{ggplotly} conversion). Defaults to \code{TRUE}.
#' @param apply_reference_lines Logical. Whether to add reference lines from
#'   input fields. Defaults to \code{TRUE}. Plots without reference-line
#'   inputs (e.g. pie, radar, ternary) should set this to \code{FALSE}.
#'
#' @return The finalized plotly figure.
#'
#' @author Jacob Martin
#' @keywords internal
#' @rdname INTERNAL_finalize_plotly
.finalize_plotly <- function(fig, input, isolate_fn,
                             facet.by = NULL,
                             include.modebar.buttons = TRUE,
                             ggplot.axis.styling = TRUE,
                             apply_axis_styling = TRUE,
                             apply_reference_lines = TRUE) {
    if (apply_axis_styling) {
        fig <- .apply_axis_post_processing(fig, input, isolate_fn,
                                           facet.by = facet.by,
                                           ggplot.axis.styling = ggplot.axis.styling)
    }
    if (apply_reference_lines) {
