...
That is the whole change. `setup_reactive_defaults()` returns `NULL` when no
...
own controls — an auto-calculated y-axis range, a regenerated list of stat
...
the stale value, then again when the client echoes the new one.

- [ ] Call `freezeReactiveValue()` on the input immediately before you update it:

    ```r
    observeEvent(input$y.data, {
        y_range <- .calculate_range(df = data(), data_col_y = input$y.data, grouping = FALSE)
        if (!is.null(y_range)) {
...
- **For a widget rebuilt by `renderUI()`, freeze inside the render function**,
  after reading the old value and before returning the widget. Putting it in an
  observer instead would freeze even when the output is suspended (hidden tab),
  and nothing would ever thaw it.
...
reposition or re-text annotations, drag the (draggable) axis titles, and slide a
continuous-colour legend (colorbar) to their heart's content. 
