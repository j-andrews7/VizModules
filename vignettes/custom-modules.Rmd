...
while `scatterPlotInputsUI()` receives the bare `id`. This ensures the base module creates its inputs in the correct namespace.
...
- By calling `scatterPlotServer(id, filtered_data)` *outside* the `moduleServer()` closure, the base module attaches to the same namespace as our UI, not a nested one.
- If we called `scatterPlotServer()` inside the `moduleServer()` block, it would create nested namespaces like `id-id-x_axis`, which wouldn't match the actual input IDs in the UI.
...
- The `minimal_wrapper.R` file in the package source contains a working example.
- The base modules (`scatterPlotInputsUI`, `scatterPlotOutputUI`, `scatterPlotServer`) are documented in the package reference.
