...
`defaults` rather than reaching for `updateTextInput()`:

```{r reactive-defaults}
labelledModuleServer <- function(id, data_reactive) {
    subset_label <- moduleServer(id, function(input, output, session) {
        reactive(if (isTRUE(input$filter_setosa)) "Setosa only" else "All species")
    })

    # The title tracks the checkbox, but the user can still type over it.
    dittoViz_scatterPlotServer(
        id, data_reactive,
        defaults = list(main = subset_label)
    )
}
```
