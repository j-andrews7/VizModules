...
- **Download the figure.** Click *Download Figure (SVG)* for a single vector SVG or *Download Figure (PNG)* for a bitmap, with every plot positioned as it appears on the page. The export is cropped tight to your panels, so you capture the whole figure without a large empty page around it. The SVG allows downstream editing as necessary, though the hope is that you'll be 95% of the way there with the formatting and syling options available in each module.
- **Automatic panel labels.** Use the *Panel labels* dropdown to add panel letters (`A`, `B`, `C`, … or lowercase `a`, `b`, `c`, …) to the top-left of each panel in the figure export. Labels are ordered the way a reader scans a figure — top-to-bottom by row, then left-to-right within a row. Choose *None* to leave the figure unlabelled.
