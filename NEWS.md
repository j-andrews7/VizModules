...
* Added save/restore of the whole Figure Builder session. A new collapsible "Session" panel lets users download the entire canvas (plot modules, their datasets, all inputs, canvas orientation, panel-label case, and each card's position/size) as a versioned JSON document and later restore it in a fresh session. Restoration replays the saved input snapshots through each module's existing `defaults` argument, so no per-module code was needed. Datasets are referenced by name; custom datasets must be re-uploaded under "Load Data" before restoring (missing datasets or unknown module types are skipped with a warning).
* Added an app-level, app-agnostic state contract with exported helpers `serialize_app_state()` / `deserialize_app_state()` (versioned via `app_state_schema_version()`) and `sanitize_input_snapshot()`, which strips transient/non-serialisable inputs (DataTables state, plotly event streams, Shiny client-data, file inputs, action-button counters) from a captured input snapshot. Any VizModules app can adopt the same contract.
