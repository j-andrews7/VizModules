...
* The `dittoViz_scatterPlot` module's custom-model feature now supports **multiple** models at once via `multiDynamicInput()`: add as many rows as you like, each with its own model type (`lm`/`glm`/`loess`/`nls`), formula, line colour, and line width, and every valid model is fitted against the active (filtered) data and overlaid as its own line (respecting faceting). Formulas are validated by the internal `.safe_build_model()` helper (fixed fit-function allow-list, `~`-topped formula requirement, syntax-tree whitelist permitting only data columns and safe math/transform terms, and result class verification), so no arbitrary code can execute. Rejected models raise a per-row warning notification. This replaces the single-model UI added earlier in this development cycle.
* **Pluggable model backend registry.** The custom model line system is now extensible: any modelling package (drc, mgcv, brms, etc.) can be plugged in without modifying core code. New exported functions: `register_model_backend()`, `get_model_backend()`, `list_model_backends()`, and `build_model_row_spec()`. Backends declare a `fit` function, a `predict` function, validated output classes, and optional extra UI `fields` that appear/hide dynamically based on the selected model type. The four built-in backends (lm, glm, loess, nls) are registered automatically at package load. Extra UI fields from backends are forwarded to `fit()` via `...`.
