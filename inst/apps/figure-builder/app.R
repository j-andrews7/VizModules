...
# Plotly chart, so we ask Plotly for an SVG of every plot and inline its vector
# markup (not a rasterised <image>) into one SVG document positioned to match
# the cards on the A4 canvas. Inlining keeps every shape as an editable vector
# object, so the download opens as fully editable artwork in Adobe Illustrator.
app_js <- HTML("
