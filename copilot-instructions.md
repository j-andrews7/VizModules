...
**vizModules** is an R package (~3.4MB, 23 R files) providing
...
- **R/** - 23 source files: `*_module_ui.R` (UI), `*_module_server.R`
  (logic), `*_module_app.R` (examples), plus utils
...
**Depends**: shiny, plotthis, dittoViz \| **Imports**: colourpicker,
plotly, shinyjs, scales, esquisse, shinyjqui \| **Suggests**: knitr,
rmarkdown
...
3.  **Test modules interactively** - run example apps (no formal test
    suite exists)
